...
// Solar2D Composite Fragment Shader: pinLight
...
const vec4 kWeights = vec4(0.2125, 0.7154, 0.0721, 1.0);
 ...
 vec4 Lighten(in vec4 base, in vec4 blend)
{
 return max(base, blend);
}

 vec4 Darken(in vec4 base, in vec4 blend)
{
 ...
 vec4 FragmentKernel(vec2 texCoord, vec4 v_ColorScale, vec4 v_UserData)
{
 ...
 vec4 resultLuminanceLow = Darken(base, 2.0 * blend);
 vec4 resultLuminanceHigh = Lighten(base, 2.0 * (blend - 0.5));

 vec4 result = ((useLuminanceLow * resultLuminanceLow) +
