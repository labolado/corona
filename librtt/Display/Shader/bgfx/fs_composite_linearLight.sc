...
// Solar2D Composite Fragment Shader: linearLight
 ...
 vec4 Add(in vec4 base, in vec4 blend)
{
 ...
 vec4 Subtract(in vec4 base, in vec4 blend)
{
 ...
 vec4 FragmentKernel(vec2 texCoord, vec4 v_ColorScale, vec4 v_UserData)
{
 ...
 float luminance = dot(blend, kWeights);
	 ...
	 vec4 resultLuminanceLow = Subtract(base, 2.0 * blend);
	 vec4 resultLuminanceHigh = Add(base, 2.0 * (blend - 0.5));
