...
// Solar2D Composite Fragment Shader: reflect
...
const vec4 kWeights = vec4(0.2125, 0.7154, 0.0721, 1.0);
 ...
 vec4 Reflect(in vec4 base, in vec4 blend)
{
 // Luminance of blend
 float luminance = dot(blend, kWeights);
	 ...
	 return result;
		...
		// step(a, b) = ((a <= b) ? 1.0 : 0.0).
		 float luminanceIsOne = step(luminance, 1.0);
		 float luminanceIsNotOne = (1.0 - luminanceIsOne);

		 vec4 resultLuminanceIsNotOne = min(base * base / (1.0 - blend), 1.0);
 ...
 vec4 FragmentKernel(vec2 texCoord, vec4 v_ColorScale, vec4 v_UserData)
{
 ...
 vec4 result = Reflect(base, blend);
