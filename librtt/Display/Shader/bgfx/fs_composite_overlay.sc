...
// Solar2D Composite Fragment Shader: overlay
 ...
 vec4 FragmentKernel(vec2 texCoord, vec4 v_ColorScale, vec4 v_UserData)
{
 ...
 // Luminance of base
 float luminance = dot(base, kWeights);

	// step(a, b) = ((a <= b) ? 1.0 : 0.0).
	 float useLuminanceLow = step(luminance, 0.5);
	 float useLuminanceHigh = (1.0 - useLuminanceLow);

 vec4 resultLuminanceLow = (2.0 * base * blend);
 vec4 resultLuminanceHigh = (1.0 - 2.0 * (1.0 - blend) * (1.0 - base));
