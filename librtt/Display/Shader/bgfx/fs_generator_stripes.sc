...
// Solar2D Generator Fragment Shader: stripes
...
{
    float pixelOnPeriod1 = (u_UserData0.x / u_ContentScale.x);
    float pixelOffPeriod1 = (u_UserData0.y / u_ContentScale.x);
    float pixelOnPeriod2 = (u_UserData0.z / u_ContentScale.x);
    float pixelOffPeriod2 = (u_UserData0.w / u_ContentScale.x);
    float pixelSum2 = (pixelOnPeriod2 + pixelOffPeriod2);
    float rotation_in_radians = radians(u_UserData1.x);
    float s = sin(rotation_in_radians);
    float c = cos(rotation_in_radians);
    float translation = (u_UserData2.x / u_ContentScale.x);
...
void main()
{
    gl_FragColor = FragmentKernel(v_TexCoord.xy, v_ColorScale, v_UserData, gl_FragCoord.xy);
}
