...
// Solar2D Composite Fragment Shader: vividLight
...
const vec4 kWeights = vec4(0.2125, 0.7154, 0.0721, 1.0);
 ...
 vec4 ColorBurn(in vec4 base, in vec4 blend)
{
 // TODO: Test for (blend == 0.0)???
 vec4 result = 1.0 - (1.0 - base) / blend;
 return result;
}

 vec4 ColorDodge(in vec4 base, in vec4 blend)
{
 // TODO: Test for (blend == 1.0)???
 vec4 result = base / (1.0 - blend);
 return result;
}

 vec4 VividLight(in vec4 base, in vec4 blend)
{
	// Luminance of blend
	 float luminance = dot(blend, kWeights);

	// step(a, b) = ((a <= b) ? 1.0 : 0.0).
	 float useLuminanceLow = step(luminance, 0.5);
	 float useLuminanceHigh = (1.0 - useLuminanceLow);

	 vec4 resultLuminanceLow = ColorBurn(base, 2.0 * blend);
	 vec4 resultLuminanceHigh = ColorDodge(base, 2.0 * (blend - 0.5));
 ...
 vec4 FragmentKernel(vec2 texCoord, vec4 v_ColorScale, vec4 v_UserData)
{
 ...
 vec4 result = VividLight(base, blend);
