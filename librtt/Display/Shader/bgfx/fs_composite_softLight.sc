...
// Solar2D Composite Fragment Shader: softLight
 ...
 vec4 FragmentKernel(vec2 texCoord, vec4 v_ColorScale, vec4 v_UserData)
{
 ...
 vec4 resultLuminanceLow = (2.0 * base * blend + base * base * (1.0 - 2.0 * blend));
 vec4 resultLuminanceHigh = (sqrt(base) * (2.0 * blend - 1.0) + 2.0 * base * (1.0 - blend));
