...
vec4 color = texture2D(u_FillSampler0, v_TexCoord.xy.st) * kWeight0;
  ...
  color += texture2D(u_FillSampler0, (v_TexCoord.xy.st + vec2(0.0, v_UserData.x) * u_TexelSize.xy)) * kWeight1;
  color += texture2D(u_FillSampler0, (v_TexCoord.xy.st - vec2(0.0, v_UserData.x) * u_TexelSize.xy)) * kWeight1;
  color += texture2D(u_FillSampler0, (v_TexCoord.xy.st + vec2(0.0, v_UserData.y) * u_TexelSize.xy)) * kWeight2;
  color += texture2D(u_FillSampler0, (v_TexCoord.xy.st - vec2(0.0, v_UserData.y) * u_TexelSize.xy)) * kWeight2;
