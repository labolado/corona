...
-- Same skybox helpers (duplicated for self-containment)
local vec3_mt = {}
vec3_mt.__index = vec3_mt
local function vec3(x,y,z) return setmetatable({x=x or 0,y=y or 0,z=z or 0}, vec3_mt) end
function vec3_mt:rot_around(axis, angle)
    local al=axis.x*self.x+axis.y*self.y+axis.z*self.z
    local ax,ay,az=axis.x*al,axis.y*al,axis.z*al
    local px,py,pz=self.x-ax,self.y-ay,self.z-az
    local rx=axis.y*self.z-axis.z*self.y; local ry=axis.z*self.x-axis.x*self.z; local rz=axis.x*self.y-axis.y*self.x
...
local function createSkybox(group, faceImages, nSlices, texSize)
    local n=nSlices; local IDX=1; local faces={}
    for i,img in ipairs(faceImages) do
        ...
        if sheet then
            ...
            for y=1,n do for x=1,n do
                local ind=x+(y-1)*n; local s=display.newImage(group,sheet,ind)
    ...
    if #faces>=6 then
        ...
        for _,s in pairs(faces[2].slices) do s.isVisible=false end
    end
    return faces, IDX
end

local rotW=vec3(-1,0,0); local rotA=vec3(0,1,0)
    ...
    for _,face in ipairs(faces) do
        for _,obj in pairs(face.slices) do
            ...
            if tl.z<0 or bl.z<0 or br.z<0 or tr.z<0 then obj.isVisible=false
                ...
                x,y=pos3d(tl);p.x1,p.y1=x,y; x,y=pos3d(bl);p.x2,p.y2=x,y-fs
                x,y=pos3d(br);p.x3,p.y3=x-fs,y-fs; x,y=pos3d(tr);p.x4,p.y4=x-fs,y
            end
        end
    end
end

-- Different skybox images for Scene B
local faceImgs = {"soil2.jpg","t1.jpg","test_gradient.png","grass1.png","desert-ground-1.jpg","castle-ground1.jpg"}

function scene:create(event)
    local g = self.view
    print("[SceneB] create — skybox + snapshot + objects")
    ...
    -- Snapshot with objects
    local workGroup = display.newGroup()
    math.randomseed(99)
    for i=1,12 do
        local c = display.newCircle(workGroup, math.random(20,280), math.random(60,350), 10+math.random(20))
        c:setFillColor(math.random(), math.random(), math.random(), 0.7)
    end
    ...
    snap:setFillColor(1, 0.9, 0.8, 0.25)
    self._snapshot = snap

    display.newText(g, "Scene B: Skybox+Snap", CX, 20, native.systemFontBold, 14):setFillColor(1)

    local btn = display.newRect(g, CX, display.contentHeight-40, 140, 36)
    btn:setFillColor(0.9, 0.3, 0.2)
    display.newText(g, "Back to A", btn.x, btn.y, native.systemFont, 14):setFillColor(1)
    btn:addEventListener("tap", function()
        composer.gotoScene("scene_switch_a", {effect="slideRight", time=300})
    end)

    self._enterFrame = function()
        if self._faces then renderSkybox(self._faces, self._IDX, 0.0003) end
